...
export default function Home() {
  ...
  // Show toast if redirected back with ?new=1 or ?error=...
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div className="stats-bar">
        <div className="stat">
          <div className="stat-num">{recipes.length}</div>
          <div className="stat-label">מתכונים</div>
        </div>
        <div className="stat">
          <div className="stat-num">{[...new Set(recipes.map((r) => r.source_name).filter(Boolean))].length || 0}</div>
          <div className="stat-label">מקורות</div>
        </div>
      </div>
      ...
      <div className="content">
        <div className="tags">
          {TAGS.map((t) => (
            <button
              key={t}
              className={`tag ${activeTag === t ? "tag-active" : "tag-inactive"}`}
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p>שתף קישור מהנייד כדי להתחיל</p>
          </div>
        ) : (
          <div className="grid">
            {filtered.map((r) => (
              <a key={r.id} href={`/recipe/${r.id}`} className="card">
                ...
                <div className="card-body">
                  ...
                  <div className="card-meta">
                    ...
                    {r.parse_status === "fallback" && (
                      <span className="meta-badge">טעון עריכה</span>
                    )}
                  </div>
...
const css = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    --cream: #F4ECD8; --cream-dark: #E8DEC4; --paper: #FAF6EE;
    --espresso: #1E1208; --olive: #5C6B2E; --olive-light: #8A9E4A;
    --terra: #B85530; --terra-light: #D4724A; --muted: #8C7B68; --card: #FFFDF7;
  }
  ...
  header { background: var(--espresso); padding: 0 1.5rem; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 50; height: 60px; }
  ...
  .toast { position: fixed; top: 70px; right: 50%; transform: translateX(50%); padding: 0.75rem 1.5rem; border-radius: 100px; font-size: 0.9rem; font-weight: 500; z-index: 300; animation: fadeIn 0.3s ease; }
  ...
  .search-bar input:focus { border-color: rgba(184,85,48,0.5); }
  ...
  .card:hover { transform: translateY(-4px); box-shadow: 0 12px 28px rgba(30,18,8,0.12); border-color: rgba(184,85,48,0.2); }
